...
for row in input_list:
    ...
    output_row[text_cat_idx] = (
        "1"
        if (
            (remove_vol_info(row[title_chn_input_idx])[-1] in ["集", "稿", "草"])
            or (remove_vol_info(row[title_chn_input_idx])[-2:] in ["詩文"])
        )
        else "0"
    )
    ...
    output_row[create_date_idx] = (
        f"{date.today().year}{str('{:0>2d}'.format(date.today().month))}{str('{:0>2d}'.format(date.today().day))}"
    )
